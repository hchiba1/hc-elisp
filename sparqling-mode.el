...
(defvar sparqling-command-default "/usr/bin/env spang2")
...
(defun insert-sparqling-template ()
  "Insert template."
  (interactive)
  (insert-statement (concat "#!" sparqling-command-default))
  (insert-statement "# @endpoint spang")
...
(defun sparqling-boost ()
  ...
  (interactive)
  (message "(#)shebang, (p)refix, (s)elect, (c)onstruct, (f)ilter, (r)egex, (o)rder, (l)imit, (h)ttp")
  (let ((c (read-char)))
    (cond
     ;; shebang line
     ((equal c ?#)
      (insert-statement (concat "#!" sparqling-command-default))
      (insert-statement "# @endpoint test")
      (backward-char 1)
      )
     ;; (p)refix
     ((equal c ?p)
